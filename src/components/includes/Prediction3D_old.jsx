...
export default function Prediction3D(props){
    ...
    const objPath = `${hostname}:${port}/get_obj_mtl?patient_id=${props.patientId}&unique_id=${props.uniqueId}&access_key=${props.accessKey}&model=${model}&file_type=obj`;
    const mtlPath = `${hostname}:${port}/get_obj_mtl?patient_id=${props.patientId}&unique_id=${props.uniqueId}&access_key=${props.accessKey}&model=${model}&file_type=mtl`;
        ...
        mtlReader.setUrl(mtlPath).then(() => {
            ...
            objReader.setUrl(objPath).then(() => {
                    ...
                    for(let i = 0;  i < objReader.getNumberOfOutputPorts(); i++) {
                        ...
                        if(model in ignore && ignore[model].indexOf(name) > -1) continue;
